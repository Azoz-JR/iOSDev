...
extension AuthenticationManager {
    ...
    // Signs in the user with Apple credentials
    @discardableResult
...
extension AuthenticationManager {
    ...
    // Connects the anonymous account to an Apple account
    @discardableResult
    func connectAnonymousToApple(tokens: SignInWithAppleResult) async throws -> AuthDataResultModel {
        let credential = OAuthProvider.credential(withProviderID: AuthProviderOption.apple.rawValue, idToken: tokens.token, rawNonce: tokens.nonce)
        return try await linkUser(credential: credential)
    }
